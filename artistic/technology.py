...
def _project_symbols(values: object) -> dict[str, tuple[int, int]]:
    if values is None:
        return {}
    if not isinstance(values, dict):
        raise ProjectError("[technology].layers must be a table")
    symbols = {}
    for name, value in values.items():
        if isinstance(value, str):
            if not re.fullmatch(r"\s*\d+\s*/\s*\d+\s*", value):
                raise ProjectError(
                    f"technology symbol has no numeric source: {value!r}")
            source = _source(value)
        elif isinstance(value, dict):
            layer = value.get("layer")
            datatype = value.get("datatype", 0)
            if (isinstance(layer, bool) or not isinstance(layer, int) or layer < 0 or
                    isinstance(datatype, bool) or not isinstance(datatype, int) or
                    datatype < 0):
                raise ProjectError(
                    f"[technology].layers.{name} requires nonnegative integer "
                    "layer and datatype values")
            source = layer, datatype
        else:
            raise ProjectError(
                f"[technology].layers.{name} must be a 'layer/datatype' string "
                "or an inline table")
        symbols[str(name)] = source
    return symbols


def resolve_project(config: dict,
                    override: str | os.PathLike[str] | None = None) -> dict:
    """Resolve a technology file and apply project-defined layer overrides."""
    settings = config.get("technology", {})
    configured_file = override or settings.get("file")
    environment_file = bool(os.environ.get("KLAYOUT_TECH_FILE")) or bool(
        os.environ.get("KLAYOUT_PATH") and os.environ.get("KLAYOUT_TECH"))
    explicit_symbols = _project_symbols(settings.get("layers"))

    tech = None
    if configured_file or environment_file or not explicit_symbols:
        path = technology_path(configured_file, config["_root"])
        tech = resolve(path)

    symbols = {}
    if tech:
        symbols.update({name: (source["layer"], source["datatype"])
                        for name, source in tech["symbols"].items()})
    symbols.update(explicit_symbols)

    configured_routing = settings.get("routing")
    if configured_routing is not None:
        if (not isinstance(configured_routing, list) or not configured_routing or
                any(not isinstance(name, str) or not name for name in configured_routing)):
            raise ProjectError("[technology].routing must be a nonempty list of layer names")
        routing = list(configured_routing)
    elif tech:
        routing = list(tech["routing"])
    else:
        raise ProjectError(
            "[technology].routing is required when no technology file is available")

    missing = [name for name in routing if name not in symbols]
    if missing:
        raise ProjectError(
            "[technology].routing references undefined layers: " + ", ".join(missing))
    top_metal = settings.get("top_metal", routing[-1] if configured_routing is not None
                           or not tech else tech["top_metal"])
    if not isinstance(top_metal, str) or not top_metal:
        raise ProjectError("[technology].top_metal must be a layer name")
    if top_metal not in symbols:
        raise ProjectError(f"[technology].top_metal references undefined layer: {top_metal}")

    return {
        "technology": tech["technology"] if tech else None,
        "symbols": {name: {"layer": source[0], "datatype": source[1]}
...
def inspect_layout(config: dict, technology: str | os.PathLike[str] | None = None,
    ...
    tech = resolve_project(config, technology)
